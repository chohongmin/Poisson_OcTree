...
// Grid_1D         MyWindow::_grid_1d;
 GRID_2D         MyWindow::_grid_2d;
 GRID_3D         MyWindow::_grid_3d;

//Array   <double>    MyWindow::_f_1d;
 ARRAY_2D<double>    MyWindow::_f_2d;
ARRAY_3D<double>    MyWindow::_f_3d;
//Array_3D<double>    MyWindow::_vf_3d_u;
//Array_3D<double>    MyWindow::_vf_3d_v;
//Array_3D<double>    MyWindow::_vf_3d_w;
...
void MyWindow::render()
{
	...
	if(_mode_df2 && _mode_df2_contour            ) m_display.contour     (_grid_2d,_f_2d,0xff0000);
	if(_mode_df2 && _mode_df2_graph              ) m_display.graph       (_grid_2d,_f_2d,0x0000ff,false);
	if(_mode_df3 && _mode_df3_isosurface_zero    ) m_display.isosurface  (_grid_3d,_f_3d,0xff0000);
	if(_mode_df3 && _mode_df3_isosurface_plus_dx ) m_display.isosurface  (_grid_3d,_f_3d,0xff0000, _grid_3d.dx);
	if(_mode_df3 && _mode_df3_isosurface_minus_dx) m_display.isosurface  (_grid_3d,_f_3d,0xff0000,-_grid_3d.dx);
	if(_mode_df3 && _mode_df3_graph_xcenter      ) m_display.graph_xconst(_grid_3d,_f_3d,0xff0000,false);
	if(_mode_df3 && _mode_df3_graph_ycenter      ) m_display.graph_yconst(_grid_3d,_f_3d,0xff0000,false);
	if(_mode_df3 && _mode_df3_graph_zcenter      ) m_display.graph_zconst(_grid_3d,_f_3d,0xff0000,false);
...
void MyWindow::display()
{
	...
	for (int i=0; i < w; i++)
	{
		...
		unsigned char r,g,b; RGB_SPLIT(rgb,r,g,b);
		*buf = b; buf++;
		*buf = g; buf++;
		*buf = r; buf++;
	}
...
LRESULT CALLBACK WndProc( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam )
{
			...
			case IDM_FILE_OPEN :
				{
					...
					if(GetOpenFileName(&ofn))
					{
						...
						if(strcmp(ext,"df3")==0)
						{
							...
							FILEIO::load(MyWindow::_grid_3d,MyWindow::_f_3d, filename);
							MyWindow::m_display.set_physical_coordinates(MyWindow::_grid_3d.xmin, MyWindow::_grid_3d.xmax,
																		 MyWindow::_grid_3d.ymin, MyWindow::_grid_3d.ymax,
																		 MyWindow::_grid_3d.zmin, MyWindow::_grid_3d.zmax);
							MyWindow::_3d_isosurface_zero();
							MyWindow::display();
						}
						...
						if(strcmp(ext,"df2")==0)
						{
							...
							FILEIO::load(MyWindow::_grid_2d,MyWindow::_f_2d, filename);

							// finding the zmin and zmax
							double zmin = 1E8;
							double zmax =-1E8;

							GRID_2D grid = MyWindow::_grid_2d;

							for(int i=0; i<grid.isize; i++)
							for(int j=0; j<grid.jsize; j++)
							{
								double z = MyWindow::_f_2d(i,j);

								zmin = MIN(z,zmin);
								zmax = MAX(z,zmax);
							}
							MyWindow::m_display.set_physical_coordinates(MyWindow::_grid_2d.xmin, MyWindow::_grid_2d.xmax,
																		 MyWindow::_grid_2d.ymin, MyWindow::_grid_2d.ymax,
																		                    zmin,                    zmax);
							MyWindow::display();
						}
					}
	...
	case WM_CHAR:
		...
		case 's':
			MyWindow::m_display.save_bitmap_file(); break;
		};
